...
void stuff(int op)
{
  ...
  {
    TrySigsafe::sigsafe_clear_interrupt();
    fprintf(stderr, "press Ctrl+C in next 2 secs ..\n");
    for (int k = 0; k < 100; ++k)
    {
      sleep_ms(20);
      TrySigsafe::sigsafe_check_interrupt(1);  // Windows MSVC needs help!
    }
...
int main(void)
{
  ...
  {
    TrySigsafe env{ TrySigsafe::SFH_DEFAULT, 0 };
    r = env.testrun( stuff, arg );
    catched_signo = env.last_catched_signal();
    fprintf(stderr, "returned sig %d and ret %d\n\n", catched_signo, r);
  }

  arg = 1;
  {
    TrySigsafe env{ TrySigsafe::SFH_DEFAULT | TrySigsafe::SFH_SEGV, 0 };
    r = env.testrun( stuff, arg );
    catched_signo = env.last_catched_signal();
    fprintf(stderr, "returned sig %d and ret %d\n\n", catched_signo, r);
  }
  ...
  for ( int k = 100; k <= 300; k += 100 )
  {
    ...
    int n_trapped = 0;
    int n_errno = 0;
    int sfhs = TrySigsafe::SFH_DEFAULT;
    int sfts = TrySigsafe::SFT_DEFAULT;
    if ( k == 200 )
      sfts = sfts;
    if ( k == 300 )
      sfts = TrySigsafe::SFT_FP_TRAPS;

    fprintf(stderr, "\n\n************************************\n\n");

    arg = k + 0;
    {
      TrySigsafe env{ sfhs, sfts };
      r = env.testrun( stuff, arg );
      catched_signo = env.last_catched_signal();
      fprintf(stderr, "returned sig %d and ret %d\n\n", catched_signo, r);
      TrySigsafe::show_fp_status();
    ...
    arg = k + 1;
    {
      TrySigsafe env{ sfhs, sfts | TrySigsafe::SFT_INEXACT | TrySigsafe::SFT_UNDERFLOW };
      r = env.testrun( stuff, arg );
      catched_signo = env.last_catched_signal();
      fprintf(stderr, "returned sig %d and ret %d\n\n", catched_signo, r);
      TrySigsafe::show_fp_status();
